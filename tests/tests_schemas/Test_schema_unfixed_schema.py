...
class TestSchemaService(unittest.TestCase):
    ...
    def test_update_schema_success(self):
        ...
        self.schema_repository.get_schema_by_id.return_value = MagicMock(isFixed=False, mentions=[], relations=[], constraints=[])

        self.schema_service.update_schema(schema_id, mentions, relations, constraints)

        self.schema_repository.add_schema_elements.assert_called_once_with(schema_id, mentions, relations, constraints)
