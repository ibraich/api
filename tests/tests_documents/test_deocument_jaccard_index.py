...
class TestJaccardIndexEndpoint(unittest.TestCase):
    ...
    def test_jaccard_index_success(self):
        ...
        response = self.app.post("/difference-calc/jaccard", headers=self.headers, data=json.dumps(payload))
        self.assertEqual(response.status_code, 200)
        data = response.get_json()
        self.assertIn("jaccard_index", data)
    
    def test_jaccard_index_missing_payload(self):
        response = self.app.post("/difference-calc/jaccard", headers=self.headers, data=json.dumps({}))
        self.assertEqual(response.status_code, 400)
        data = response.get_json()
        self.assertIn("error", data)
    
    def test_jaccard_index_invalid_format(self):
        response = self.app.post("/difference-calc/jaccard", headers=self.headers, data="invalid json")
        self.assertEqual(response.status_code, 400)
        data = response.get_json()
        self.assertIn("error", data)
