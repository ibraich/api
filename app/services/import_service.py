...
class ImportService:
    ...
    def import_pet_documents(
        ...
        schema = self._schema_service.get_schema_by_id(schema_id)

        for pet_document in pet_documents:
            self._import_pet_document(pet_document, schema, project_id, user_id)

        return {"count": len(pet_documents), "success": True}

    def _import_pet_document(
        self, pet_document: any, schema: any, project_id: int, user_id: int
    ):
